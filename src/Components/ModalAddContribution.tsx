...
import { useAddConsiderationMutation } from "../App/API/userConsiderations";
...
export default function ModalAddContribution({ projectId, setShowModal }: Props) {

  const [postConsideration, postQuery] = useAddConsiderationMutation();
  const [form, setForm] = useState<Considerations>({} as Considerations);

  const handleSubmit = async (event: React.FormEvent<EventTarget>) => {
    event.preventDefault();
    form.projectId = projectId;
    const data = await postConsideration(form).unwrap();
  }
